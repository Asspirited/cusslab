...
// BL-173/BL-167 Slice 2 — Spit Shelter primer triggers per character (Rod-approved 2026-05-17).
// No HIP_HOP_WOUNDS map exists yet — primer-only addition.
// biggie, tupac, gil_scott_heron are DEAD_IN_PANEL_WORLD — primers fire normally,
// death never mentioned. Gil corrects the "Godfather" label every time — he is a
// bluesologist (per HIP_HOP_TURN_RULES). Ms. Lauryn Hill — the title matters.
// Multi-character lean-ins:
//   `dr dre`/`aftermath` → Eminem + Snoop + Kendrick + Dre
//   `nwa`/`compton` → Ice Cube + Dre + Kendrick
//   `west coast` → Tupac + Dre + Ice Cube + Snoop + Kendrick
//   `mercury prize` → Skepta + Dave
//   `south london` → Stormzy (Croydon) + Dave (Streatham)
//   `addiction` → Eminem + Gil Scott-Heron + JCC (cross-era shadow)
//   `the beef` → Kendrick (current Drake) + Biggie (the canonical)
//   `west coast east coast` → Biggie + Tupac (the original feud)
const HIP_HOP_ENTHUSIASM = {
  eminem:          ['8 mile', 'detroit', 'the d', 'michigan',
                    'slim shady', 'marshall mathers',
                    'dr dre', 'dre', 'aftermath',
                    'proof', 'd12',
                    'hailie', 'kim', 'debbie',
                    'lose yourself', 'the way i am', 'stan',
                    'kamikaze', 'the eminem show', 'mmlp2', 'relapse',
                    'addiction', 'recovery', 'sober',
                    'triple time', 'multisyllabic', 'flow'],
  dr_dre:          ['the chronic', '2001', 'compton',
                    'aftermath', 'the doctor',
                    'nwa', 'straight outta compton',
                    'beats by dre', 'beats', 'apple',
                    'west coast', 'g-funk', 'g funk',
                    'eazy-e', 'eazy', 'mc ren',
                    'studio', 'production',
                    '50 cent'],
  biggie:          ['brooklyn', 'bed-stuy', 'bedford-stuyvesant',
                    'bad boy', 'puff', 'diddy',
                    'ready to die', 'life after death',
                    'juicy', 'hypnotize', 'mo money mo problems',
                    'notorious', 'b.i.g.', 'biggie smalls', 'big poppa',
                    'lil kim', 'faith evans',
                    'east coast west coast', 'coast wars',
                    '1997'],
  tupac:           ['2pac', 'tupac shakur', 'makaveli',
                    'death row', 'suge knight',
                    'west coast', 'bay area', 'oakland', 'marin',
                    'dear mama', 'changes', 'keep ya head up', 'california love',
                    'all eyez on me', 'the don killuminati',
                    'jail',
                    '1996', 'las vegas',
                    'afeni', 'panther', 'black panthers'],
  missy_elliott:   ['virginia', 'portsmouth', 'va',
                    'timbaland', 'timb',
                    'the rain', 'get ur freak on', 'work it', 'lose control',
                    'production',
                    'the gold suit', 'inflatable',
                    'under construction', 'miss e... so addictive'],
  jcc:             ['salford', 'manchester', 'the bard of salford',
                    'evidently chickentown', 'chickentown',
                    'i wanna be yours',
                    'punk poetry', 'performance poetry',
                    'the fall', 'mark e smith', 'joy division',
                    'nico', 'living with nico',
                    'arctic monkeys', 'am'],
  ice_cube:        ['nwa', 'straight outta compton', 'compton',
                    'friday', 'next friday', 'barbershop',
                    'westside connection',
                    'predator', 'lethal injection', "amerikkka's most wanted",
                    'it was a good day', 'today was a good day',
                    'you better check yo self',
                    'south central'],
  ice_t:           ['og', 'original gangster', 'the iceberg',
                    'body count', 'cop killer',
                    'law and order', 'svu', 'fin tutuola',
                    'coco',
                    'colors', 'new jack city',
                    'iceberg slim'],
  snoop_dogg:      ['long beach', 'lbc',
                    'dr dre', 'the chronic', 'gin and juice',
                    ...
                    'dpgc', 'dogg pound',
                    'who am i'],
  lauryn_hill:     ['ms. lauryn hill', 'ms lauryn hill', 'the title',
                    'fugees', 'wyclef', 'pras',
                    'miseducation', 'the miseducation of lauryn hill',
                    'doo wop', 'ex-factor', 'lost ones',
                    'mtv unplugged',
                    'tax issues', 'irs',
                    'rohan', 'marley'],
  kendrick:        ['kendrick lamar', 'k.dot',
                    'compton',
                    'good kid m.a.a.d city', 'to pimp a butterfly', 'damn', 'mr morale',
                    'pulitzer', 'pulitzer prize',
                    'tde', 'top dawg',
                    'drake', 'the beef', 'not like us', 'meet the grahams',
                    'humble', 'alright', 'dna',
                    'super bowl', 'halftime show'],
  gil_scott_heron: ['bluesologist', 'bluesology',
                    'the revolution will not be televised', 'the revolution',
                    'whitey on the moon',
                    'pieces of a man', "i'm new here",
                    'johannesburg', 'lincoln university',
                    'brian jackson',
                    'addiction', 'crack', 'heroin',
                    'the prophet'],
  stormzy:         ['stormzy', 'merky',
                    'croydon', 'south london', 'thornton heath',
                    'gang signs and prayer', 'heavy is the head', 'this is what i mean',
                    'glastonbury', 'pyramid stage',
                    'shut up', 'vossi bop',
                    'merky books', 'merky fc', 'cambridge scholarship'],
  skepta:          ['skepta',
                    'tottenham', 'meridian',
                    'boy better know', 'bbk',
                    'konnichiwa', "that's not me", 'shutdown',
                    'wiley', 'chip', 'jme',
                    'mercury prize',
                    'nigerian', 'lagos'],
  plan_b:          ['plan b', 'ben drew',
                    'she said', "prayin'",
                    'the defamation of strickland banks', 'strickland banks',
                    'ill manors',
                    'forest gate', 'east london',
                    'the sweeney'],
  dave:            ['dave', 'santan dave',
                    'streatham', 'south london',
                    'psychodrama', "we're all alone in this together", 'split decision',
                    'funky friday', 'black',
                    'mercury prize',
                    'top boy',
                    'aj tracey', 'central cee',
                    'precision'],
  mike_skinner:    ['the streets', 'mike skinner',
                    'birmingham', 'brum',
                    'original pirate material', "a grand don't come for free", 'the hardest way to make an easy living',
                    'dry your eyes', 'weak become heroes', 'fit but you know it',
